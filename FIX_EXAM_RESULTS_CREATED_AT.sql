-- Fix missing 'created_at' column in exam_results table
-- This script adds the missing 'created_at' column and updates existing records
...
BEGIN
    ...
    -- Check if other commonly expected columns exist
    IF NOT EXISTS (
        SELECT 1 
        FROM information_schema.columns 
        WHERE table_schema = 'public' 
    ...
    -- Handle marks_obtained type change (needs special handling due to generated column)
    IF EXISTS (
        SELECT 1 
        FROM information_schema.columns 
        WHERE table_schema = 'public' 
        ...
        ALTER TABLE public.exam_results 
        ALTER COLUMN marks_obtained TYPE DECIMAL USING marks_obtained::DECIMAL;
        
        -- Recreate the generated column
        ALTER TABLE public.exam_results 
        ADD COLUMN percentage DECIMAL GENERATED ALWAYS AS (ROUND((marks_obtained / max_marks) * 100, 2)) STORED;
        
        RAISE NOTICE '✅ Changed marks_obtained to DECIMAL type and recreated percentage column';
    END IF;
...
SELECT 
    'EXAM_RESULTS_FINAL_STRUCTURE' as check_type,
    column_name,
    data_type,
    is_nullable,
    column_default
