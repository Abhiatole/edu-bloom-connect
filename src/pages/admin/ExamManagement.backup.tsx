...
// Interface for manual entry of marks
interface MarkEntry {
  studentId: string;
  studentName: string;
  rollNumber?: string;
...
const ExamManagement = () => {
  ...
  const [newSubjectId, setNewSubjectId] = useState<string | null>(null);  const [exportingExamId, setExportingExamId] = useState<string | null>(null);
  const [uploadingResults, setUploadingResults] = useState(false);
  const [csvError, setCsvError] = useState<string | null>(null);  const [manualEntryMode, setManualEntryMode] = useState(false);
  ...
  const handleSubjectChange = (subjectId: string) => {
    ...
    try {
      ...
      if (examForm.subjectId === 'other') {
        ...
        // We'll proceed with exam creation in the next render        return;
    ...
    // Set loading state
    setUploadingResults(true);    // Show initial progress toast
    toast({
      title: "Uploading...",
  ...
  const downloadSampleCsv = () => {
    ...
    try {
      ...
      if (error.message) {
        ...
        if (error.code === "PGRST116") {
          errorMessage = "Database relationship issue. Check your database setup.";
        } else if (error.code === "42P01") {
          errorMessage = "Missing table. Please run the database setup script.";
        } else {
    ...
    try {
      ...
      setExamForm(prev => ({
        ...prev,        subjectId: generatedSubjectId
      }));
  ...
  // Function to handle student search  const handleSearchStudents = (query: string) => {
  ...
  const submitManualEntries = async () => {
    if (markEntries.length === 0 || !selectedExam) {
      toast({
        title: "Error",
            ...
            <DialogContent>              <DialogHeader>
              ...
              {subjects.length === 0 && (
                <Alert className="mt-2 mb-4">
                  ...
                  <AlertTitle>Database Setup Required</AlertTitle>
                  <AlertDescription>
                    The subject list could not be loaded. This may be due to a database setup issue.
                  </AlertDescription>
                </Alert>
              )}
              ...
              <form onSubmit={handleCreateExam} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <Input 
                        ...
                        required={examForm.subjectId === 'other'}
                      />
                      <Button 
                        type="button" 
                        variant="outline" 
                ...
                <div>
                  ...
                  <Input
                    ...
                    required
                  />
                </div>                <Button type="submit" className="w-full">Create Exam</Button>
              </form>
            </DialogContent>
          </Dialog>
          ...
          <Dialog open={markResultsOpen} onOpenChange={setMarkResultsOpen}>
            ...
            <DialogContent>              <DialogHeader>
              ...
              <div className="space-y-4">
                <div>
                  <Label htmlFor="examSelect">Select Exam *</Label>                  <Select value={selectedExam} onValueChange={setSelectedExam}>
                    <SelectTrigger>
                ...
                {!manualEntryMode ? (
                  ...
                  <>
                    ...
                    <div className="mb-4">
                      <Label htmlFor="studentSearch">Search Student (by Name or Roll Number)</Label>
                        ...
                        <div className="border rounded-md max-h-40 overflow-y-auto mb-2">
                          {filteredStudents.map(student => (
                            ...
                            >
                              ...
                              <div className="text-gray-500">Class {student.class_level}</div>
                            </div>
                          ))}
                        </div>
                      ) : searchQuery ? (
                        <p className="text-sm text-gray-500 mb-2">No matching students found</p>
                      ) : null}
                        {/* Mark Entries */}
                      <div className="border rounded-md p-2 mb-4">
                        <h3 className="font-medium mb-2">Student Marks</h3>
                        {markEntries.length === 0 ? (
                          <p className="text-sm text-gray-500">No students added yet. Search above to add students.</p>
                        ) : (
                          <div className="space-y-2">
                            <div className="grid grid-cols-12 gap-2 text-sm font-medium border-b pb-1">
                            ...
                            {markEntries.map(entry => {
                              // Find the student to get their class level
                              const student = students.find(s => s.id === entry.studentId);
                              
                              return (
                                <div key={entry.studentId} className="grid grid-cols-12 gap-2 items-center">
                                  ...
                                  <div className="col-span-2 text-xs">
                                    {student ? `Class ${student.class_level}` : '-'}
                                  </div>
                    ...
                    <Button 
                      ...
                      className="w-full" 
                      disabled={uploadingResults || markEntries.length === 0 || !selectedExam}
                    >
                      {uploadingResults ? (
                        <>
                          <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
