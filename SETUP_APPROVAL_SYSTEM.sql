-- =====================================================
-- USER APPROVAL SYSTEM SETUP WITH HISTORY TRACKING
-- =====================================================
...
-- Step 2: Create a view to combine all users for easier querying
-- Using the ACTUAL column names from the TypeScript types
-- student_profiles: approved_by_teacher_id -> doesn't exist, only approval_date exists
-- teacher_profiles: approved_by_admin_id -> doesn't exist, only approval_date exists
...
SELECT 
    ...
    user_id,
    'TEACHER' as role,
    COALESCE(employee_id, id::text, 'unknown') as identifier,
    NULL as class_level,
    CASE WHEN approval_date IS NOT NULL THEN 'APPROVED' ELSE 'PENDING' END as status,
    NULL as approved_by,  -- Column doesn't exist
    approval_date,
...
-- Step 6: Create a function to handle approvals with logging
...
DECLARE
    ...
    user_name TEXT;
    approver_name TEXT;
BEGIN    -- Get approver name from all_users_view or set default
    ...
    ELSIF p_user_type = 'teacher' THEN
        SELECT COALESCE(employee_id, id::text) INTO user_identifier
